...
import nhutlm2.fresher.demochathead.Utils.ChatHeadUtils;
...
public class MaximizedArrangement<User extends Serializable> extends ChatHeadArrangement {
    ...
    private ChatHeadManager<User> manager;
    ...
    private void pointTo(ChatHead<User> activeChatHead) {
        UpArrowLayout arrowLayout = getArrowLayout();
    ...
    public void onCapture(ChatHeadManager container, ChatHead activeChatHead) {
        if (!activeChatHead.isSticky()) {
            container.removeChatHead(activeChatHead.getUser(), true);
        }
    }
    ...
    public Integer getHeroIndex() {
        int heroIndex = 0;
        List<ChatHead<User>> chatHeads = manager.getChatHeads();
        int i = 0;
    ...
    public void removeOldestChatHead() {
        for (ChatHead<User> chatHead : manager.getChatHeads()) {
            ...
            if (!chatHead.isSticky() && chatHead != currentChatHead) {
                manager.removeChatHead(chatHead.getUser(), false);
                break;
            }
        }
    }
